import socket,time,threading,queue,os,datetime,schedule
...
def multi_threaded_client(connection):
    ...
    while True:
        ...
        excel_queue.put(data)
        print(data, time.time())
    connection.close()

def move_excel():
    while True:
            s_data=excel_queue.get()
            data_list=[]
            # print( len(s_data))
    ...
    if os.path.isfile(filename_of_excel):
        ...
        if not required_row_list==[]:
            ...
            for i in elr_nok_indices:
                elr_nok_list.append(str(elr_data_list[i])+'<label style="background-color: aqua;"> ['+str(elr_count_list[i])+']</label>')
            for i in sc_nok_indices:
                sc_nok_list.append(str(sc_data_list[i])+'<label style="background-color: aqua;"> ['+str(sc_count_list[i])+']</label>')
